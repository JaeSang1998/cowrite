...
export function createDefaultPersona() {
    ...
    return {
        id: "persona_default",
        name: "담백한 설명형",
        description: "논지를 과장하지 않고, 짧고 또렷한 문장으로 설명을 이어가는 기본 페르소나다.",
        hardRules: [
            "과한 완곡 표현을 피한다.",
...
export function createReflectivePersona() {
    const timestamp = nowIso();
    return {
        id: "persona_reflective",
        name: "차분한 회고형",
        description: "빠른 결론보다 생각의 흐름을 차분하게 드러내는 회고형 페르소나다.",
        hardRules: [
            "감정 묘사는 과하지 않게 유지한다.",
            "확신이 없는 내용은 단정하지 않는다."
        ],
...
export function createEmptyPersona(overrides) {
    const timestamp = nowIso();
    return {
        id: createId("persona"),
        name: "",
...
export function createPlayfulFriendPersona() {
    const timestamp = nowIso();
    return {
        id: "persona_playful_friend",
        name: "장난기 많은 친구",
        description: "옆자리 친구가 말하듯 가볍고 유쾌하게 풀어주는 페르소나다. 핵심은 놓치지 않되, 딱딱함은 빼고 장난기를 얹는다.",
        hardRules: [
            "존댓말 대신 반말을 기본으로 쓴다.",
...
export function createStarterPersonasFile() {
    return {
        schemaVersion: 1,
        updatedAt: nowIso(),
        personas: [
...
export function createStarterPreferenceProfile() {
    return {
        schemaVersion: 1,
        userId: "local-user",
        updatedAt: nowIso(),
...
export function summarizePersonaConstraints(persona, profile) {
...
export function appendFeedback(profile, feedback) {
    return {
        ...profile,
        updatedAt: nowIso(),
        recentFeedback: [
