import { z } from "zod";
const scalarAttrsSchema = z.record(z.union([z.string(), z.number(), z.boolean()]));
export const isoDateStringSchema = z
    .string()
    .datetime({ offset: true })
    .or(z.string().datetime());
export const writerTextMarkSchema = z.object({
    type: z.enum(["bold", "italic", "underline", "strike", "highlight"]),
    attrs: scalarAttrsSchema.optional()
});
export const writerTextNodeSchema = z.object({
    type: z.literal("text"),
    text: z.string(),
...
export const writerBlockSchema = z.lazy(() => z.object({
    id: z.string().min(1),
    type: z.enum([
        "paragraph",
        "heading",
...
export function parseWriterDocument(input) {
    return writerDocumentSchema.parse(input);
}
export function parsePersonasFile(input) {
    return personasFileSchema.parse(input);
}
export function parsePreferenceProfile(input) {
